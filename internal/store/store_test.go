...
func TestSessionsAreScopedToTheirClient(t *testing.T) {
	...
	sess, err := st.CreateSession(ctx, owner.ID, "mine", "", "", "", true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeleteSessionIsScopedAndCascades(t *testing.T) {
	...
	sess, err := st.CreateSession(ctx, owner.ID, "doomed", "", "", "", true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTranscriptRoundTrip(t *testing.T) {
	...
	sess, err := st.CreateSession(ctx, client.ID, "", "", "", "", true)
	if err != nil {
		t.Fatal(err)
	}
...
func newTestSession(t *testing.T, st *Store) *Session {
	...
	sess, err := st.CreateSession(ctx, client.ID, "test", "", "", "", true)
	if err != nil {
		t.Fatal(err)
	}
	return sess
