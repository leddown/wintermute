...
# WINTERMUTE_CONVERT_STAGING=/fast/wintermute-staging

# Where a release is assembled and converted, when that should not be the
# repository itself. Empty puts it in <repository>/.staging, which needs no
# second drive and is the right answer on an SSD.
#
# It is worth setting when the repository is on a spinning disk. A conversion
# reads the safetensors and writes the GGUF at the same time, and one head
# serving both streams spends the job seeking between them; on separate disks
# they are two sequential transfers, which is what a drive is good at. The
# finished model is then copied across once — hashed on the way past, so it
# costs one pass rather than two — and appears under its real name only when it
# is whole.
#
# It needs room for the release and the GGUF at once, roughly twice the model,
# and the service user has to be able to write there. Under systemd that means
# a ReadWritePaths= line for it as well, and RequiresMountsFor= if it is its own
# disk.
